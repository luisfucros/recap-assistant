...
- FR-16.4 **User and document languages are independent.** The chat language may differ from a book's language (e.g. a Spanish-speaking user reading a German book). The assistant **answers in the user's preferred language regardless of the document's language**; verbatim quotes/citations are shown in the document's original language (translated only if the user asks).
