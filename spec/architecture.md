...
- **Terraform modules** live in `infra/terraform/` and organize infrastructure by concern: `vpc`, `ecr` (implemented), plus stubs for `rds`/`postgres`, `elasticache`/`redis`, `ecs-api`, `ecs-ingestion`, `qdrant`, `alb`/`cloudfront`, `s3`, `monitoring`, `iam`/`security`. No `secrets` module — config is variables → task env. **No state-backend module** — remote state uses an already-created S3 (+ lock table) backend the operator supplies via `terraform init -backend-config`; this stack never creates the state bucket or DynamoDB lock table.
