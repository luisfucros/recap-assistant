...
- **Topical relevance:** a cheap-tier classifier decides whether the request concerns the user's reading/documents; off-topic requests get a polite redirect **in the reader's language** and never reach the tools/LLM answer path. Voice notes and images about reading are on-topic (they are how the reader asks).
