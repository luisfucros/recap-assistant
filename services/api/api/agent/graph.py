...
* **extract_memory** — a cheap-tier :class:`~api.agent.schemas.MemoryClassification`
  judge over the reader's latest message: a salient, non-``summary`` verdict
  (a durable personal fact/preference/habit) is saved immediately via
  :class:`~api.services.memory_service.MemoryService` — no confirmation
  needed, unlike a page-range summary, since it's the reader's own statement
  about themselves. Best-effort: a failure here never discards the turn's
...
def build_agent_graph(
    *,
    ...
    async def extract_memory(state: AgentState) -> dict:
        ...
        reader's latest message; a salient, non-``summary`` verdict is saved
        immediately via :class:`~api.services.memory_service.MemoryService` —
        no confirmation needed, unlike a page-range summary (FR-4.6), since
        this is the reader's own statement about themselves, not a generated
        recap that could misrepresent the source text. Best-effort: a
        classification or write failure never breaks the turn, since the
        answer has already been generated by the time this runs.
        """
        try:
            prompt_obj = prompts.get("memory_classify", "v1")
            prompt = prompt_obj.render(message=_latest_user_text(state))
            with (
                tracer.span(
                    "extract_memory",
